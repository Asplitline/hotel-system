      ...
      <li class="h-item" v-for="item in filterGoods" :key="item.id">
        ...
        <div class="h-info">
          ...
          <el-popconfirm title="确定要添加商品到购物车吗？" @confirm="handleAddCart(item)">
            <button class="h-cart" slot="reference"><i class="iconfont icon-iconfontcart"></i>加入购物车</button>
          </el-popconfirm>
        </div>
      </li>
...
export default {
  ...
  methods: {
    // 映射 Vuex 中的 setCurrentItem mutation
    ...mapMutations(['setCurrentItem']),
    // 映射 Vuex 中的 fetchMyCart action
