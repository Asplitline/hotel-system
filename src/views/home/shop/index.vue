      ...
      <el-container class="main">
        <el-main>
          <div class="main-header">
            <shop-nav :data="item" v-for="item in goodsCategory" :key="item.id" @h-tag="handleTag" :active="active" />
          </div>
...
export default {
  ...
  computed: {
    ...mapGetters(['getCategoryById', 'getMiniCategory', 'getUserById', 'getRoomById', 'goodsCategory']),
    fItem() {
      if (this.hasSearch) {
        return this.item.filter((i) => i.name.indexOf(this.keyWord?.trim()) !== -1)
      } else {
        return this.active === '' ? this.item : this.item.filter((i) => i.type === this.active)
      }
    }
    // categories() {
