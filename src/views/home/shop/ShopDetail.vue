...
export default {
  ...
  mixins: [hMixin],
  methods: {
    // 使用 mapMutations 映射了 setCurrentItem 方法到组件内，可在方法中调用 mutation
    ...mapMutations(['setCurrentItem']),
    // 使用 mapActions 映射了 fetchMyCart 和 fetchAllCategory 方法到组件内，可在方法中调用 action
    ...mapActions(['fetchMyCart', 'fetchAllCategory']),
    // 定义 bindIMG 方法
    bindIMG,
    // 定义 async 函数 handleAddCart，用于添加购物车
    async handleAddCart(item) {
      ...
      // 如果成功添加购物车，则调用 fetchMyCart 方法更新购物车列表
      success && this.fetchMyCart()
    },
    // 定义函数 goto，用于跳转到购物车页面
    goto() {
  ...
  computed: {
    // 使用 mapState 和 mapGetters 映射了组件需要的状态和 getter，可直接在组件内使用
    ...mapState({
      item: 'currentItem',
      myCart: 'myCart',
  ...
  // 在组件挂载后调用 fetchAllCategory 方法，获取所有商品类型
  mounted() {
