  ...
  <div class="cart">
    <template v-if="myCart.length">
      ...
      <div class="cart-tb">
        <el-row class="cart-bd" v-for="i in myCartList" :key="i.id">
          ...
          <el-col :span="6" class="cart-col handle">
            ...
            >
              <a href="javascript:;" class="link-del" slot="reference"
                >移出购物车</a
              >
            </el-popconfirm>
      ...
      <div class="cart-tool">
        <el-popconfirm
          title="确定要清空购物车吗？"
          @confirm="clearShopCar"
          style="margin-right: auto"
        >
          <a href="javascript:;" class="cart-clear" slot="reference"
            >清空购物车</a
          >
        </el-popconfirm>
    ...
    <el-empty class="md-empty" v-else>
      <template #description>
        <p>购物车为空</p>
        <router-link :to="{ name: 'Shop' }" style="color: red; margin-left: 4px"
          >前往商城
        </router-link>
      </template>
    </el-empty>
...
export default {
  ...
  methods: {
    ...
    async purchaseAll() {
      ...
      const ids = this.allItem.map((i) => {
        return { id: i.id, num: i.num, name: i.name }
      })
      const computeCartCount = {}
      cartList.forEach((i) => {
        if (computeCartCount[i.goodsId]) {
          computeCartCount[i.goodsId]++
        } else {
          computeCartCount[i.goodsId] = 1
        }
      })
      let hasError = false
      Object.keys(computeCartCount).map((key) => {
        const realItem = ids.find((i) => i.id === key)
        if (computeCartCount[key] > realItem.num) {
          hasError = true
          this.$message.error(
            `${realItem.name} 余量不足, 当前购买 ${computeCartCount[key]}, 实际余量 ${realItem.num} `
          )
        }
      })
      if (!hasError) {
        ...
        setTimeout(() => {
          this.$message.warning('请将多余的商品移除购物车后，重新购买')
        }, 1000)
      }
      console.log('computeCartCount :', computeCartCount)
      console.log('ids :', ids)
    },
    async clearShopCar() {
