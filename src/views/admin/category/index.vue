<template>
	<el-card class="room">
		...
		<el-table :data="tableData" style="width: 100%" max-height="650px" row-key="id"
			...
			<el-table-column label="操作" min-width="100">
				<template v-slot="{ row }">
					...
					<el-link type="danger" @click="deleteById(deleteCategory, fetchCategory, row.id, '商品类型')">删除商品类型
					</el-link>
				</template>
			</el-table-column>
		</el-table>
		...
		<!--  -->
		<el-dialog :title="categoryForm.flag === 0 ? '添加商品类型' : '修改商品类型'" :visible.sync="dialogVisible" width="30%"
			...
			<el-form :model="categoryForm" :rules="categoryRules" ref="categoryForm" size="small" label-width="100px">
				...
				<el-form-item label="父级商品类型" prop="pid">
					<el-select v-model="categoryForm.pid">
						<el-option v-for="i in parentCategory" :key="i.id" :value="i.id" :label="i.name">
						</el-option>
					</el-select>
				</el-form-item>
