  ...
  <el-card class="item">
    ...
    <el-form v-model="searchForm" :inline="true" size="small">
      <el-row type="flex" justify="space-between">
        ...
        <el-col :span="4">
          <el-form-item>
            <el-button type="success" @click="showDialog(0)"> 新增商品 </el-button>
          </el-form-item>
        </el-col>
      </el-row>
    </el-form>
    <el-table :data="tableData" style="width: 100%" max-height="650px" class="demo-table">
      ...
      <el-table-column prop="typeInfo.name" label="商品分类" min-width="100"> </el-table-column>
      ...
      <el-table-column label="操作" min-width="120">
        <template v-slot="{ row }">
          <el-link :type="row.status === 0 ? 'success' : 'warning'" @click="toggleStatus(row)">
            {{ row.status === 0 ? '上架商品' : '下架商品' }}
          </el-link>
          <el-link type="primary" @click="showDialog(1, row)"> 修改商品</el-link>
          <el-link type="danger" @click="deleteById(deleteItem, fetchItem, row.id, '用户')"> 删除商品 </el-link>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog
      :title="flag === 0 ? '添加商品' : '修改商品'"
      :visible.sync="dialogVisible"
      width="30%"
      class="a-dialog"
      @close="clearDialog('itemForm')"
    ...
    >
      <el-form :model="itemForm" :rules="itemRules" ref="itemForm" size="small" label-width="100px">
        ...
        <el-form-item label="描述" prop="description">
          <el-input type="textarea" v-model="itemForm.description" placeholder="商品描述"> </el-input>
        </el-form-item>
      </el-form>
    ...
    <el-pagination
      @size-change="handleSizeChange(fetchItem, $event)"
      @current-change="handleCurrentChange(fetchItem, $event)"
...
export default {
  data() {
    return {
      ...
      itemRules: {
        ...
        address: [{ required: true, message: '请输入商品产地', trigger: 'blur' }],
  ...
  methods: {
    ...
    search() {
      // 点击搜索按钮触发的函数
      this.fetchItem(this.searchForm.name) // 调用fetchItem函数，传入搜索框的内容
    },
    async fetchItem(keyword) {
      // 获取商品列表的函数，参数为关键字
      const { list, total } = await _api.getItemList({ ...this.query, keyword }) // 调用接口获取商品列表数据
      this.total = total // 将总数赋值给total
      this.tableData = list.map((i) => {
        // 对每个商品数据进行处理，添加分类信息和状态信息
        const typeInfo = this.getCategoryById(i.type)
        const statusInfo = this.goodsState.find((j) => j.value === Number(i.status))
    ...
    handleAvatarSuccess(res, file) {
      // 上传商品图片成功后的回调函数，将图片url保存到itemForm中
      this.$set(this.itemForm, 'url', res)
    },
    showDialog(flag, row) {
      // 打开弹窗的函数，根据flag值决定是添加还是修改
      this.flag = flag
      if (flag === 0) {
        // 添加商品
      } else {
    ...
    submitDialog(formName, flag) {
      // 提交弹窗表单的函数，根据flag值决定是添加还是修改
      this.$refs[formName].validate(async (valid) => {
        if (!valid) return // 如果表单验证不通过，则不提交
        if (flag === 0) {
          // 添加商品
          ...
          const { success } = await _api.addItem(this[formName]) // 调用接口添加商品
          this.handleSuccess(success, '添加', this.fetchItem) // 处理添加成功后的操作
          this.dialogVisible = false // 关闭弹窗
        } else if (flag === 1) {
          // 修改商品
          delete this[formName].typeInfo
          delete this[formName].statusInfo
          const { success } = await _api.editItem(this[formName]) // 调用接口修改商品
          this.handleSuccess(success, '修改', this.fetchItem) // 处理修改成功后的操作
          this.dialogVisible = false // 关闭弹窗
        }
      })
    },
    async toggleStatus(row) {
      ...
      this.handleSuccess(success, status === 0 ? '下架' : '上架', this.fetchItem)
    }
  },
  // 引入Vuex中的getters和state
  computed: {
    ...
    categories() {
      ...
      return parent
        ...
        .filter((i) => i.children.length > 0) // 过滤掉没有子分类的父分类
    }
  },
  mounted() {
    // 获取所有用户信息
