  ...
  <el-card class="order">
        ...
        <el-col :span="6">
          <el-form-item label="客房状态">
            <el-select v-model="searchForm.state" placeholder="请选择客房类型" clearable>
              <el-option v-for="item in orderInfo" :key="item.id" :label="item.value"
                :value="item.id">
              </el-option>
            </el-select>
          </el-form-item>
    ...
    <el-table :data="filterTableData" style="width: 100%" max-height="650px">
      ...
      <el-table-column label="操作" min-width="120">
        <template v-slot="{row}">
          <el-link type="success" @click="handleRoom(row,3)" :disabled="row.state!==1">
            同意入住</el-link>
          <el-link type="danger" @click="handleRoom(row,2)" :disabled="row.state!==1">
            拒绝入住</el-link>
          <el-link type="primary" @click="handleRoom(row,0)" :disabled="row.state!==3">
            退房处理
          </el-link>
        </template>
...
import { orderInfo, roomType, ACCEPT, REJECT, FINISH } from '@static'
...
export default {
  ...
  methods: {
    ...
    async handleRoom(data, flag) {
      data = deepClone(data)
      switch (flag) {
        case ACCEPT:
          this.confirmInfo(data, flag, '将同意客户入住', this.fetchOrder)
          break
        case REJECT:
          this.confirmInfo(data, flag, '将拒绝客户入住', this.fetchOrder)
          break
        case FINISH:
          this.confirmInfo(data, flag, '将为客人退房', this.fetchOrder)
          break
      }
    },
    confirmInfo(data, flag, info, callback) {
      ...
      if (this.hasFilter) {
        ...
        if (data && name && name.trim().length > 0) {
          data = data.filter((item) => {
            return item.room.name.indexOf(name) !== -1
          })
        }
        console.log(data)
