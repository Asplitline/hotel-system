  ...
  <div class="login">
    <div class="login-wrap">
      <div class="ms-login" :class="{ active: isLogin }">
        ...
        <el-form :model="loginForm" :rules="rules" ref="loginForm" label-width="0px" class="ms-content" key="loginForm">
          ...
          <el-form-item prop="password">
            <el-input type="password" placeholder="密码" v-model="loginForm.password" @keydown.enter.native="submitForm('loginForm', 0)">
      ...
      <div class="ms-register" :class="{ active: !isLogin }">
        ...
        <el-form :model="registerForm" :rules="rules" ref="registerForm" label-width="0px" class="ms-content" key="registerForm">
          <el-form-item prop="username">
          ...
          <el-form-item prop="checkCode">
            <el-input v-model="registerForm.checkCode" placeholder="验证码">
              ...
              <el-button
                class="toggle-button"
                slot="append"
                :icon="loadBtn[loadFlag ? 1 : 0].icon"
          ...
          <div class="ms-btn">
            <el-row :gutter="40">
              ...
              <el-col :span="12">
                <el-button type="primary" @click="submitForm('registerForm', 1)">注册 </el-button>
              </el-col>
            </el-row>
          </div>
        </el-form>
...
export default {
  ...
  methods: {
    ...mapMutations(['setCurrentUser']), // 引入vuex中的setCurrentUser方法
    submitForm(formName, flag) {
      // 提交表单方法，需要传入表单名称和标志位
      this.$refs[formName].validate(async (valid) => {
        // 使用表单的$refs验证表单是否合法
        if (!valid) return // 如果不合法，直接返回
        if (flag === LOGIN) {
          // 如果标志位是LOGIN
          const success = await _api.login(convertURL(this[formName])) // 调用api中的login方法验证用户登录是否成功
          const data = await _api.getUserByUsername({
            // 调用api中的getUserByUsername方法获取用户信息
            username: this[formName].username
          })
          if (success) {
            // 如果登录成功
            this.$message.success('登录成功，跳转中....') // 显示成功信息
            this.setCurrentUser(data) // 调用setCurrentUser方法更新vuex中的用户信息
            if (Number(data.level) === 1) {
              // 如果用户level为1
              this.$router.push({ name: 'Admin' }) // 跳转到Admin页面
            } else if (Number(data.level) === 0) {
              // 如果用户level为0
              this.$router.push({ name: 'Index' }) // 跳转到Index页面
            }
          } else {
            // 如果登录不成功
            this.$message.error('账号或密码错误') // 显示错误信息
          }
        } else if (flag === REGISTER) {
          // 如果标志位是REGISTER
          if (this.checkCode !== this[formName].checkCode) {
            // 验证码不匹配
            return this.$message.error('验证码错误') // 显示错误信息
          }
          const res = await _api.register(this.handleData(this[formName])) // 调用api中的register方法注册用户
          if (res) {
            // 如果注册成功
            this.handleSuccess(res.success, '注册') // 调用handleSuccess方法显示成功信息
            this.toggleForm(formName) // 切换表单
          } else {
            // 如果注册不成功
            this.$message.error('账号已存在') // 显示错误信息
          }
        }
      })
    },
    toggleForm(formName) {
      // 切换表单的方法
      this.isLogin = !this.isLogin // 切换isLogin的值
      this.$refs[formName].resetFields() // 重置表单
      this.initEmail() // 初始化email
    },
    initEmail() {
      // 初始化email的方法
      this.loadFlag = false // loadFlag为false
      this.showEmail = true // showEmail为true
    },
    async getValidateCode() {
      // 获取验证码的方法
      this.loadFlag = true // loadFlag为true
      try {
        this.checkCode = await _api.checkCode({
          // 调用api中的checkCode方法获取验证码
